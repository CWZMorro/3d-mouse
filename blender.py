    ...
    def modal(self, context, event):
        if event.type == "TIMER":
            try:
                ...
                # Global Reset Signal (Snaps to 0,0,0)
                ...
                if mode == "viewport_touch" and rv3d:
                    if touches == 1:
                        ...
                        pitch = Quaternion(
                            rv3d.view_rotation @ Vector((1, 0, 0)), dy * 0.01
                        )
                    ...
                    if self._center_alpha is None:
                        self._center_alpha, self._center_beta, self._center_gamma = (
                            alpha,
                            beta,
                            gamma,
                        )
                    ...
                    d_alpha = self.apply_deadzone(
                        self.get_shortest_angle_diff(alpha, self._center_alpha),
                        self._deadzone,
                    )
                    d_beta = (
                        self.apply_deadzone(
                            self.get_shortest_angle_diff(beta, self._center_beta),
                            self._deadzone,
                        )
                        * inv_y
                    )
                    d_gamma = (
                        self.apply_deadzone(
                            self.get_shortest_angle_diff(gamma, self._center_gamma),
                            self._deadzone,
                        )
                        * inv_x
                    )
                    ...
                    sx, sy = (
                        float(msg.get("sensX") or 50) / 2000.0,
                        float(msg.get("sensY") or 50) / 2000.0,
                    )
